 ...
 *
 * タスクの5つ（見守りの入り切り・間隔・任せること・特にしてほしいこと・対象の人）もここに
 * あります。見守りは裏でAIを動かし続けるので、費用を抑えたくなったその場で切れることが、
...
export function createSettings({ values = {}, manager = false, file = null } = {}) {
  ...
  return {
    ...
    /**
     * タスクの決め事。実行係（`autoTaskRunner.js`）は見守りのたびにこれを読むので、
     * 画面で切った直後の回から、裏では何も送らなくなります。`enabled` が切るのは見守りだけで、
     * 画面から押す「AIに起こさせる」は、切ってあっても走ります。
     */
    get autoTasks() {
      return Object.freeze({
