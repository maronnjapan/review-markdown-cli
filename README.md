...
- その資料について分かったことを1件ずつ積み上げるコンテキストメモ（背景・決定・制約・未決）
...
読み取りコンテキストが「この資料はこう読む」を1枚に整えたものなのに対して、コンテキストメモは「このとき、こう分かった」を1件ずつ積み上げるものです。
...
| 種類 | 何を書くか | AI の読み方 |
| --- | --- | --- |
| 背景 | なぜこの資料があるか。どこから来たか | 前提として読む。指摘の対象にはしない |
| 決定 | もう決めたこと | その論点を指摘し直さない |
...
`contextNotes` は、その資料について残したメモです。1件も残していない文書には現れません。`kind` は `background`（背景）／`decision`（決定）／`constraint`（制約）／`question`（未決）のいずれかで、AI の読み方がこれで変わります。`source` が `chat` のメモは、AIチャットの回答から残したものです。残した順に並び、食い違うメモは後のほうを採るとAIへ伝えます。レビューMarkdownを出力すると、「コンテキストメモ」として書き出します。
