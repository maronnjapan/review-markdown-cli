...
実行すると、指摘ごとに対象箇所を特定したコメント候補が並びます。
...
各スキルの「詳細を見る」を押すと、そのスキルの本文をパネル内で読めます。
表示するのはプロンプトへ載せるものと同じ本文なので、`SKILL.md` を別のエディタで開き直さずに、選ぶ前に何を見るスキルか確かめられます。
...
プロンプトへ載せるのは先頭12000文字までで、`references/` などの参照ファイルは読み込みません。
