...
この2つに加えて、AIパネルに残したコンテキストメモも前提として読みます。
「決定」と残した論点は蒸し返さず、「制約」を破っている箇所は指摘します。
同じスキルと読み手でも、残したメモによって出る指摘は変わります（「コンテキストメモ」を参照）。
...
```
prompts/readingContext.js  読み取りコンテキスト・コンテキストメモ・読み手ペルソナ・レビューコメントの枠
...
documentReview.js   レビュースキルの選択、読み手ペルソナの組み直し、AIレビューの実行
