...
- 走り書きで書いた読み手を AI が立場・前提知識・目的へ組み直す読み手ペルソナ
...
組み直した結果はコメントと同じ自動保存でレビューファイル（`.review/<target>.review.json`）の `persona` に保存し、次に開いたときもそのまま使えます。
「ペルソナを削除」で消せます。
...
`persona` は、その文書の読み手ペルソナです。`input` はレビュアーが書いた走り書き、それ以外はAIが組み直した結果で、`assumptions` はAIが補った前提です。設定していない文書には現れません。レビューMarkdownを出力すると、「読み手ペルソナ」として書き出します。
