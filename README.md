...
読み取りコンテキストを設定していれば、指摘の解釈にも同じ前提を使います。
...
| 指摘の件数、渡す本文の量、見取り図に載せる見出し数、残せるメモの件数と長さ | `src/aiLimits.js` |
...
`contextNotes` は、その文書について残したメモです。1件も残していない文書には現れません。`kind` は `background`（背景）／`decision`（決定）／`constraint`（制約）／`question`（未決）のいずれかで、AI の読み方がこれで変わります。`source` が `chat` のメモは、AIチャットの回答から残したものです。残した順に並び、食い違うメモは後のほうを採るとAIへ伝えます。レビューMarkdownを出力すると、「コンテキストメモ」として書き出します。
