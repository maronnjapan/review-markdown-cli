...
src/aiService.js         翻訳・会話セッション・指摘コメント配置の制御
...
commentPlacement.js 指摘コメントの配置依頼とコメント候補の描画
...
「指摘の配置」から追加したコメントには `"source": "ai"` が付きます。対象箇所はAIが特定し、追加の判断はレビュアーが行っています。
...
- `source`: `ai` のとき、対象箇所をAIが特定したコメント。
